...
var newArray = [];
var allConversations = [lesson03, topic03];
var chosenTopic;
...
function dialogueInit(){
	newArray = [];
	pickTopic();
		console.log("\n\n\ntopic chosen: " + chosenTopic.topic);
	pickConversation();
...
function printMenu() {
	var selectList = document.getElementById("menuTopic");
	...
	for (var i = 0; i < allConversations.length; i++) {
		...
		option.value = allConversations[i].topic;
		option.text = allConversations[i].topic;

		if (option.value == chosenTopic.topic) {
...
// 	// make array with list of possible topics
// 	// do loop 
